...
\author{
 ...
 Nombre Asesor\textsuperscript{2} % TODO: reemplazar con nombre completo del asesor
}
...
\address{
	...
	\textsuperscript{2 }TODO: institución, ciudad, país - correo@institucion.mx % TODO: reemplazar con datos del asesor
}
...
\abstract{
The Mexico City Metropolitan Area (ZMVM) hosts one of the largest public transit
...
a free and open-source infrastructure that integrates official GTFS feeds and
OpenStreetMap collaborative cartography to build a unified multimodal topological
model of the ZMVM, exposed as a REST service in GeoJSON format.  Built on top of
this infrastructure, the \textit{VFTModel} analytical engine computes three
network efficiency indicators: spatial coverage, deviation factor, and capillary
strength.  Results show that Milpa Alta borough records only 11.75\,\% pedestrian
coverage and that the average inter-borough trip covers 54\,\% more distance than
the geometric minimum.  The architecture is replicable in any city with available
GTFS data and OpenStreetMap coverage.
}
...
de red metropolitana unificado, y cómo este modelo habilita análisis topológicos
de nivel académico sin barreras técnicas de entrada.
...
relacional espacial PostgreSQL equipada con la extensión PostGIS.  Apimetro ingiere
los archivos GTFS oficiales de SEMOVI y los somete a un proceso de
...
México---, la API utiliza de forma estratégica la cartografía colaborativa de
\textbf{OpenStreetMap} como fuente de validación cruzada y datos complementarios.
Mediante consultas SQL espaciales nativas, Apimetro unifica ambas fuentes y genera
un modelo de datos coherente.  La segunda capa corresponde al motor analítico
\textit{VFTModel}, que consume los servicios REST resultantes para
...
\begin{enumerate}[nosep, leftmargin=1.5em]
  ...
  \item \textbf{Factor de Desviación ($DF$):} cociente entre la distancia real de
    ...
    100~rutas interalcaldías arroja $\bar{DF}=1.545$, lo que significa que cada
    viaje recorre, en promedio, 54\,\% más de lo necesario.  El caso extremo
    (Milpa Alta $\to$ Santa Catarina Yecahuizotl) alcanza $DF=2.52$.
...
grafo de \textbf{11\,115~nodos} interconectados por \textbf{45\,679~aristas}
(25\,000 de servicio y 20\,000 de transbordo), abarcando \textbf{11~sistemas} de
transporte, todo en un volumen de 38~MB
de datos procesados.  Estos resultados demuestran que la API de Apimetro permite
